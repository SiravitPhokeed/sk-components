...
Manage the field value with `useState`. Use `behavior` to switch between
single-line, multi-line, and textarea modes.
  ...
  <PropsTableRow prop="appearance" required="always">
    ...
    <TableCell>
      ...
      - Keep the appearance consistent across Text Fields. For example, use
        `"outlined"` for form fields and `"filled"` for search bars, rather
        than mixing both in the same context.
...
<VersionsTable>
  <VersionsTableRow version="v4.0.0">
    `name` introduced for form submission. `type` introduced for native HTML
    input types. `inputAttr.type` replaced. `canClear` removed.
  </VersionsTableRow>
