...
<VersionsTable>
  <VersionsTableRow version="v4.0.0">
    `name` introduced for form submission. `value` type narrowed from `any` to
    `string`. Prop injection replaced with React Context. `disabled` and
    `required` introduced. The trigger now exposes its state to assistive
    technologies.
  </VersionsTableRow>
