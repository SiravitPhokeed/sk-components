...
export interface UseAnimatedPopoverOptions {
  ...
  /**
   * Optional callback called when the popover is closed via ESC key or
   * click outside. This is called before the exit animation starts.
   *
   * - Useful for controlled components that need to update their `open` state.
   * - Optional.
   */
  onClose?: () => void;
}
...
export function useAnimatedPopover(
  ...
  const { exitingClass, exitAnimationName, wrapperRef, onClose } = options;
